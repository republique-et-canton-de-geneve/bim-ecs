...
export class SystemRunner<TPayload, TReturnType = any> implements Disposable, Debuggable {
  ...
  readonly #schedulerIterable: Scheduler<TPayload>
  ...
  constructor(
    ...
    this.#schedulerIterable = new this.ecs.scheduler(ecs.world, { systemId: this.id })
    this.#iterator = this.#schedulerIterable[Symbol.asyncIterator]()

    this.#unsubscribeSchedulerDisposedEvent = ecs.world.bus.subscribe(
      SCHEDULER_DISPOSED_EVENT,
      ({ host }) => {
  ...
  public async run() {
    ...
    while (!this.#disposed && this.#running) {
      ...
      try {
        // System processing
        let result = this.ecs.system(this.ecs.world, {
  ...
  private publishStateEvent(key: EcsEvent<{ id: number; time: number }>, additionalData?: any) {
    const time = Date.now()
    this.ecs.world.bus.publish(
      key,
      additionalData ? { id: this.id, time, ...additionalData } : { id: this.id, time },
    )
  }
  ...
  [Symbol.dispose]() {
    if (!this.#disposed) {
      ...
      this.#schedulerIterable[Symbol.dispose]()
